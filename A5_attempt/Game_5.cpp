...
pair<int, int> Game::find_x(int find_p) //returns 0 (not found), -1 (white found), -2 (black found)
{
	...
	for (int i = -4; i <= 4; i++)
	{
		...
		for (int j = y_max; j >= y_min; j--)
		{
			if (board_state[i + 5][j + 5] == color)
			{
				...
				{ //white markers
					if (color == WHITE_MARKER)
					{
						p_count.first++;
						count_continuous = 0;
					}
					//black markers
					else if (color == BLACK_MARKER)
					{
						p_count.second++;
						count_continuous = 0;
					}
			...
			else if (board_state[i + 5][j + 5] == WHITE_MARKER)
			{
				color = WHITE_MARKER;
				count_continuous = 1;
			}
			else if (board_state[i + 5][j + 5] == BLACK_MARKER)
			{
				color = BLACK_MARKER;
				count_continuous = 1;
			}
	...
	for (int i = 4; i >= -4; i--) //Y
	{
		...
		for (int j = x_left; j <= x_right; j++)
		{
			...
			else if (board_state[j + 5][i - j + 5] == WHITE_MARKER)
			{
				color = -1;
				count_continuous = 1;
			}
			else if (board_state[j + 5][i - j + 5] == BLACK_MARKER)
			{
				color = -2;
				count_continuous = 1;
			}
